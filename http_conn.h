...
public:
    ...
    // 读缓冲区不能太小，否则无法容纳一行，则下一次读的数据就会被冲掉
...
public:
    void init(int sockfd, const sockaddr_in &addr); // 初始化 新接收的连接
    void close_conn();
    bool read();  // 非阻塞的读
    bool write(); // 非阻塞的写
...
public:
    static int m_epollfd;    // 所有的 socket 上的事件都被注册到同一个epoll对象中
    static int m_user_count; // 统计用户的数量

private:
    int m_sockfd;                 // 该HTTP连接的socket
    sockaddr_in m_address; // c++，可以去掉 struct
